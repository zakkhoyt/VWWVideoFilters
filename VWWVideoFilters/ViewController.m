...
- (void)setupFilter;
{
    ...
    if (self.filterType == GPUIMAGE_FILECONFIG)
    {
        ...
        pipeline = [[GPUImageFilterPipeline alloc] initWithConfigurationFile:[[NSBundle mainBundle] URLForResource:@"SampleConfiguration" withExtension:@"plist"]
                                                                       input:videoCamera output:(GPUImageView*)self.gpuImageView];
    ...
    else
    {
        ...
        else if (self.filterType == GPUIMAGE_MOTIONDETECTOR)
        {
            ...
            [self.gpuImageView addSubview:faceView];
            faceView.hidden = YES;
            ...
            [(GPUImageMotionDetector *) filter setMotionDetectionBlock:^(CGPoint motionCentroid, CGFloat motionIntensity, CMTime frameTime) {
                if (motionIntensity > 0.01)
                {
                    ...
                    CGSize viewBounds = weakSelf.gpuImageView.bounds.size;
...
- (IBAction)updateFilterFromSlider:(id)sender;
{
    ...
    switch(self.filterType)
    {
        ...
        case GPUIMAGE_LOCALBINARYPATTERN:
        {
            ...
            [(GPUImageLocalBinaryPatternFilter *)filter setTexelWidth:(multiplier / self.gpuImageView.bounds.size.width)];
            [(GPUImageLocalBinaryPatternFilter *)filter setTexelHeight:(multiplier / self.gpuImageView.bounds.size.height)];
        }; break;
        default: break;
    }
}
                 ...
                 andOrientation:(UIDeviceOrientation)curDeviceOrientation
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        CGRect previewBox = self.gpuImageView.frame;
